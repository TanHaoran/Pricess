...
@IonicPage({
  name: 'page-e-fall-bed',
  segment: 'page-e-fall-bed'
})
@Component({
  selector: 'page-e-fall-bed',
  templateUrl: 'e-fall-bed.html',
})
export class EFallBedPage {
  public userInfo: any = {};
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad EFallBedPage');
  }
  ...
  checkInput() {
    ...
    if (!this.commonCheck.isMinAndMax(this.ddzcData.HospNumber, 0, 10)) {
  ...
  reportEvent() {
    ...
    this.ddzcDataTemp.DdzcType = '243';
    ...
    // this.ddzcDataTemp.FileName = JSON.stringify(this.fileNames);
...
//开关关闭时清除地面状态里的数据
  selectGroundState(item){
    if(item.ECodeValue === '275') {
      if(item.isChecked) {
        _.each(this.dataSource.groundState, function (item) {
          if(item.ECodeValue !=='275') {
            item.isDisabled = true;
            item.isChecked = false;
          }
